//David Centeno
//CS5008 Data Structures Algorithims and Applications
//Homework 5
//Program prints preorder, inorder, and postorder traversals
...
void preorder (node* np) {
	...
	if(np == NULL)
		return;
	//Starts function by printing roots data starting with parent node,left node, then right node
	printf("%c",np->data);
	//recursively traverses down the left of the tree until null is found
	preorder(np->left);
	//recursively traverses down until left is root is null and goes into right side of node
	preorder(np->right);	
}
void inorder (node* np) {
	...
	//recursively traverses down the left of the tree until null is found
	inorder(np->left);
	//Prints the bottom left most root data starting with left node, parent node, then right node
	printf("%c", np->data);
	//recursively traverses down the right side of the tree once the left side is complete
	inorder(np->right);
}
...
void postorder (node* np) {
	...
	if(np == NULL) 
		return;
	//recursively traverses down the left side of the tree until null is found 
	postorder(np->left);
	//recursively traversed down the right side of the tree once the left side of the tree meets null
	postorder(np->right);
	//prints the root data starting at the bottom of the left side of the tree beginning with the left node, right node, then parent node
	printf("%c", np->data);
}
